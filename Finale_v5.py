...
if result in (SAT,OPTIMUM):
    maxTime=value(score)+1
    #get values first
    instantiation_vars = ""
    instantiation_vals = ""
    for i in range(ntasks):
        stri = str(i)
        j = greedyOrder.index(i)
        instantiation_vars+= 'task_machine'+stri+","
        instantiation_vals+= str(value(tasks_machines[j]))+","
        instantiation_vars+= 'start_task'+stri+","
        instantiation_vals+= str(value(tasks_starts[j]))+","
    total_instantiation = instantiation_vars[:-1]+":"+instantiation_vals[:-1]
    print('initialisation result : ')
    maxend=0
    ...
    print(total_instantiation)
    clear()
    tasks_machines = []
    tasks_starts= []
    for i in range(ntasks):
        tasks_machines.append(Var(dom=set(tasks_array[i][1]), id='task_machine'+str(i)))
        tasks_starts.append(Var(dom=range(maxTime-tasks_array[i][0]), id='start_task'+str(i)))
    satisfy(
    ...
    result=solve(options="-t=180s -ins="+total_instantiation)
    print(result)
    if result in (SAT, OPTIMUM):
